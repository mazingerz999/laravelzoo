...
class WSDLDocument extends DOMDocument
{
    ...
    protected function createArrayType($sType)
    {
        ...
        $sName = $sType . 'Array';
        //if (array_key_exists($sName, $this->aCreatedTypes) == true) {
        if (array_key_exists($sName, $this->aCreatedTypes) == true) {
            return $sName;
        }
        // avoid recursion
    ...
    protected function createComplexType($sClass)
    {
        ...
        $documentationClase = $oReflection->getDocComment();
        $propiedades = $this->getTagCommentProperty($documentationClase, 'property');
        foreach ($propiedades as $propiedad) 
        {
            $oElement = $this->createElementNS(self::NS_XSD, 'element');
            $oAll->appendChild($oElement);
            $oElement->setAttribute('name', $propiedad['nombre']);
            $oElement->setAttribute('type', $this->createType($propiedad['tipo']));
            $oElement->setAttribute('minOccurs', 0);
            $oElement->setAttribute('maxOccurs', 1);
        }

        //NUEVO - Para meter únicamente las @property en nuestras clases del modelo de Laravel
        if($oReflection->isSubclassOf("Illuminate\Database\Eloquent\Model"))
        {
            return;
        }
        ...
        foreach ($oReflection->getProperties() as $oProperty) {
            ...
            if (
                ...
                $comentario = $this->getTagComment($sComment, 'var');
                $sType = reset($comentario);
    ...
    protected function createMessage(ReflectionMethod $oMethod)
    {
        ...
        // output part
        $com = $this->getTagComment($sComment, 'return');
        $sType = (string) reset($com);
        if ($sType != 'void' && $sType != '') {
            $oPart = $this->createElementNS(self::NS_WSDL, 'part');
    ...
    protected function getTagCommentProperty($sComment, $sTagName)
    {
        $aValue = array();
        foreach (explode("\n",  $sComment) as $sLine) {
            $sPattern = "/^\*\s+@(.[^\s]+)\s+(.[^\s]+)\s+.(.[^\s]+)/";
            $sText = trim($sLine);
            $aMatch = array();
            preg_match($sPattern, $sText, $aMatch);
            //var_dump($aMatch);
            if (count($aMatch) > 2 && $aMatch[1] == $sTagName) {
                $aValue[] = array("tipo"=>$aMatch[2], "nombre"=> $aMatch[3]);
            }
        }
        return $aValue;
    }
